...
const DEFAULT_CHIRPSTACK_TENANT_NAME = "ChirpStack";
...
function buildBaseCodec() {
  return `function decodeUplink(input) {
  return {
    data: {},
    warnings: ["Base demo profile. Utility-specific WattWise profiles are generated by provisioning."]
  };
}`;
}
...
async function ensureChirpStackTenant(chirpStackPool) {
  const tenantName = process.env.CHIRPSTACK_TENANT_NAME ?? DEFAULT_CHIRPSTACK_TENANT_NAME;
  const existingResult = await chirpStackPool.query(
    "SELECT id FROM tenant WHERE name = $1 ORDER BY created_at DESC LIMIT 1",
    [tenantName],
  );
  ...
  await chirpStackPool.query(
    `
      INSERT INTO tenant (
        id, created_at, updated_at, name, description, can_have_gateways,
        max_device_count, max_gateway_count, private_gateways_up,
        private_gateways_down, tags
      )
      VALUES (
        $1::uuid, NOW(), NOW(), $2, $3, true,
    ...
    [
      tenantId,
      tenantName,
      "Tenant provisioned for the WattWise thesis demo.",
    ],
  );
...
async function ensureChirpStackApplication(chirpStackPool, tenantId, applicationName) {
  const existingResult = await chirpStackPool.query(
  ...
  if (existing?.id) {
    if (existing.tenant_id !== tenantId) {
      await chirpStackPool.query(
        "UPDATE application SET tenant_id = $2::uuid, updated_at = NOW() WHERE id = $1::uuid",
  ...
  await chirpStackPool.query(
    `
      INSERT INTO application (
        id, tenant_id, created_at, updated_at, name, description, mqtt_tls_cert, tags
      )
      VALUES (
        $1::uuid, $2::uuid, NOW(), NOW(), $3, $4, NULL, '{}'::jsonb
      )
    `,
    [
      applicationId,
      tenantId,
      applicationName,
      "Application provisioned for the WattWise thesis demo.",
    ],
  );
...
async function ensureBaseChirpStackDeviceProfile(chirpStackPool, tenantId, profileName) {
  const existingResult = await chirpStackPool.query(
    "SELECT id FROM device_profile WHERE name = $1 ORDER BY created_at DESC LIMIT 1",
    [profileName],
  );
  ...
  if (existingId) {
    await chirpStackPool.query(
      "UPDATE device_profile SET tenant_id = $2::uuid, updated_at = NOW() WHERE id = $1::uuid",
      [existingId, tenantId],
    );
  ...
  await chirpStackPool.query(
    `
      ...
      VALUES (
        $1::uuid, $2::uuid, NOW(), NOW(), $3, 'EU868', '1.0.3', 'A',
        'default', 'JS', 3600, 1,
        true, false, false, '{}'::jsonb, $4,
        true, $5, '{}'::jsonb, false,
        'eu868', false, 0, NULL, NULL, NULL,
        NULL, $6::jsonb, NULL, '', 0,
        '{}'::smallint[]
      )
    `,
    [
      ...
      profileName,
      buildBaseCodec(),
      "Base profile provisioned for the WattWise thesis demo.",
      JSON.stringify({
        ts003_f_port: 202,
        ts004_f_port: 201,
        ts005_f_port: 200,
...
async function resolveChirpStackTargets(chirpStackPool) {
  ...
  const tenantId = await ensureChirpStackTenant(chirpStackPool);
  const applicationId = await ensureChirpStackApplication(chirpStackPool, tenantId, applicationName);
  const baseDeviceProfileId = await ensureBaseChirpStackDeviceProfile(chirpStackPool, tenantId, profileName);
